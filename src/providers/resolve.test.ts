...
describe('mapListingExchange', () => {
  it('maps every IBKR venue the statements actually use', () => {
    ...
    expect(mapListingExchange('BVME.ETF')).toEqual({ kind: 'prefixed', prefix: 'BIT' })
    expect(mapListingExchange('BVME')).toEqual({ kind: 'prefixed', prefix: 'BIT' })
    ...
    expect(mapListingExchange('SBF')).toEqual({ kind: 'prefixed', prefix: 'EPA' })
